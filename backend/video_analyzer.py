    ...
    def analyze_single_frame(self, frame):
        ...
        try:
            ...
            if face_results.multi_face_landmarks:
                ...
                metrics["eye_contact"] = 8 if eye_contact else 5

                # Detect smile
                is_smiling = self._detect_smile(face_landmarks)
                metrics["expressiveness"] = 8 if is_smiling else 6

                # Base confidence from face detection
                metrics["confidence"] = 7.5
                metrics["professional_presence"] = 7.5
                metrics["engagement"] = 7 if eye_contact else 5
            else:
                # No face detected
                metrics["eye_contact"] = 3
                metrics["confidence"] = 4
                metrics["engagement"] = 4
                metrics["expressiveness"] = 5
                metrics["professional_presence"] = 4
            ...
            if pose_results.pose_landmarks:
                ...
                metrics["body_language"] = 8 if good_posture else 5
                metrics["stability"] = 7
            else:
                metrics["body_language"] = 5
                metrics["stability"] = 6

            # Calculate overall score
            metrics["overall_score"] = sum([
                metrics["eye_contact"] * 0.20,
                metrics["confidence"] * 0.25,
                metrics["body_language"] * 0.20,
                metrics["expressiveness"] * 0.10,
                metrics["stability"] * 0.10,
                metrics["professional_presence"] * 0.10,
                metrics["engagement"] * 0.05
            ])

            # Generate feedback
            feedback = self._generate_single_frame_feedback(metrics)
            ...
            return {
                ...
                "feedback": {
                    "strengths": [],
                    "areas_for_improvement": ["Could not analyze image properly"],
                    "specific_tips": ["Ensure good lighting and face clearly visible"]
                }
            }

    def _generate_single_frame_feedback(self, metrics):
        """Generate feedback for single frame analysis"""
        feedback = {
            "strengths": [],
        ...
        # Eye Contact
        if metrics["eye_contact"] >= 7:
            feedback["strengths"].append("Good eye contact with camera - shows confidence")
        else:
            feedback["areas_for_improvement"].append("Eye contact could be improved")
            feedback["specific_tips"].append("Look directly at the camera lens, not the screen")

        # Confidence
        if metrics["confidence"] >= 7:
            feedback["strengths"].append("Confident appearance and demeanor")
        else:
            feedback["areas_for_improvement"].append("Could project more confidence")
            feedback["specific_tips"].append("Maintain steady gaze and relaxed facial expression")

        # Body Language
        if metrics["body_language"] >= 7:
            feedback["strengths"].append("Professional posture and body language")
        else:
            feedback["areas_for_improvement"].append("Posture needs attention")
            feedback["specific_tips"].append("Sit up straight with shoulders back")

        # Expressiveness
        if metrics["expressiveness"] >= 7:
            feedback["strengths"].append("Good facial expressions - appears engaged")
        else:
            feedback["areas_for_improvement"].append("More facial expressiveness would help")
            feedback["specific_tips"].append("Smile naturally when appropriate")

        # Professional Presence
        if metrics["professional_presence"] >= 7:
            feedback["strengths"].append("Strong professional presence")
        else:
            feedback["specific_tips"].append("Ensure good lighting and neat appearance")

        # If no strengths found, add generic one
        if not feedback["strengths"]:
            feedback["strengths"].append("Face clearly visible for analysis")

        # Always add general tip
        feedback["specific_tips"].append("For better analysis, record a short video showing natural movement")
